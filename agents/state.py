...
from typing import TypedDict, Optional  # 从 typing 模块导入当前文件后续要用到的对象


class AgentState(TypedDict):  # 定义 AgentState 类，用来把这一块相关的状态和行为组织在一起
    """
    Agent 状态类
    ...
    category: Optional[str]  # 继续执行这一行具体逻辑，推动当前函数或代码块往下运行
    ...
    news_list: list[dict]  # 继续执行这一行具体逻辑，推动当前函数或代码块往下运行
